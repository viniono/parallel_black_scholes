...
int i=0;
    while (!feof(file))
    {
        ...
        fscanf(file, 
               ...
               &blackScholes_inputs[i].K,
               &blackScholes_inputs[i].T,
               &blackScholes_inputs[i].skip,
               &blackScholes_inputs[i].sigma,
               &blackScholes_inputs[i].r,
               &blackScholes_inputs[i].skip2);       
               i++;
    ...
    for(i; i>0;i--){
        double call = BS_CALL( blackScholes_inputs[i].S,
                               blackScholes_inputs[i].K,
                               blackScholes_inputs[i].T,
                               blackScholes_inputs[i].r,
                               blackScholes_inputs[i].sigma);
        double put = BS_PUT( blackScholes_inputs[i].S,
                               blackScholes_inputs[i].K,
                               blackScholes_inputs[i].T,
                               blackScholes_inputs[i].r,
                               blackScholes_inputs[i].sigma);
